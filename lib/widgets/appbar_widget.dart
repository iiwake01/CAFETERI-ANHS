...
class AppbarWidget extends BaseWidget<MainController> with PreferredSizeWidget {
  ...
  Widget build(BuildContext context) {
    return Obx(() {
      if (controller.getAppBarEnum() == AppBarEnum.home || controller.getAppBarEnum() == AppBarEnum.profile) return PreferredSize (
        preferredSize: preferredSize,
        child: HomeAppBarWidget()
      );
      else if(controller.getAppBarEnum() == AppBarEnum.shop) return PreferredSize (
        preferredSize: preferredSize,
        child: const ShopAppBarWidget()
      );
  ...
  Size get preferredSize {
    return Size.fromHeight(120); //controller.getPreferredSize();
  }
}
